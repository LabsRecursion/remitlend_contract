...
impl OracleVerifier {
    ...
    pub fn report_remittance(
        ...
        let is_monitored: bool = env.storage().instance()
    ...
    fn verify_operator(env: &Env, operator: &Address) {
        ...
        for i in 0..count {
            let authorized: Address = env.storage().instance().get(&DataKey::OracleOperators(i)).unwrap();
